...
#define DEFAULT_SERIAL_COM_DI_PIN 10
#define DEFAULT_SERIAL_COM_RO_PIN 9 
#define DEFAULT_SERIAL_COM_BUS &Serial1
#define DEFAULT_SERIAL_COM_CONTROL_PIN 8
...
void ESP32_Midea_RS485Class::Update()
{
    ...
    }else
    {
      ...
      //set mode flags
      SentData[11] =   (DesiredState.AuxHeat_Turbo<<1)|(DesiredState.Echo_Sleep<<0)|(DesiredState.Swing<<2)|(DesiredState.Vent*0x88);
      //set timer start
      SentData[9] =  CalculateSetTime(State.TimerStart);      
      //set timer stop
      SentData[10] =  CalculateSetTime(State.TimerStop);
      //unknown -> 0
      SentData[12] =  0;
      SentData[13] =  0xFF-SentData[1];
      SentData[15] =  PROLOGUE;
